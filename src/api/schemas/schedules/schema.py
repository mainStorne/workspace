...
class ScheduleCreate(_ScheduleCreate):
    ...
    def validate_bound_datetime(self, lowest: time, highest: time) -> None:
        ...
        if intake_finish_time < lowest:
            raise ValueError(f"intake_finish_time is less then lowest time {lowest.strftime('%H:%m')}")  # noqa: TRY003
        if intake_finish_time > highest:
            raise ValueError(f"intake_finish_time is more then highest time {highest.strftime('%H:%m')}")  # noqa: TRY003

        intake_start_time = self.intake_start.time()
        if intake_start_time < lowest:
            raise ValueError(f"intake_start_time is less then lowest time {lowest.strftime('%H:%m')}")  # noqa: TRY003
        if intake_start_time > highest:
            raise ValueError(f"intake_start_time is more then highest time {highest.strftime('%H:%m')}")  # noqa: TRY003
